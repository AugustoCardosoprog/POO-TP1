...
public class mainEj5yEj12Dos {
    public static void main(String[] args) {

    int opcion;
    Scanner scanner = new Scanner(System.in);
    ListaTarea2 listaDeTareas = new ListaTarea2();
    ...
    do{
        ...
        switch (opcion) {
            ...
            case 10:
                ...
                break;
            case 11:
                //Establecer fecha recordatorio
                int numero6 = selecionarNumeroTarea(listaDeTareas);
                if (numero6!=-1) {
                    System.out.print("Ingrese una fecha (en formato AAAA-MM-DD): ");
                    Scanner scannerCuatro = new Scanner(System.in);
                    String fechaComoTexto = scannerCuatro.nextLine();
                    if (fechaComoTexto.matches("\\d{4}-\\d{2}-\\d{2}")){
                        LocalDate fecha = LocalDate.parse(fechaComoTexto);
                        listaDeTareas.setFechaLimite(numero6,fecha);
                        System.out.println("Fecha recordatorio establecida.");
                        mostrarTodo(listaDeTareas,numero6);
                    }else {
                        System.out.println("Formato incorrecto.");
                    }
                }
    ...
    public static String mostrar(ListaTarea2 listaTarea,int numero) {
        String acumulador;
    ...
    public static void mostrarArrayList(ArrayList<Tarea2> list){
        int i=-1;
        if (!list.isEmpty()){
            for (Tarea2 elemento: list) {
                i++;
    ...
    public static String strCompleta(ListaTarea2 listaTarea,int numero){
        String acumulador;
        if (!listaTarea.esVacia()) {
    ...
    public static String strPorVencer(ListaTarea2 listaTarea,int numero){
        String acumulador;
        if (!listaTarea.esVacia()) {
            if (listaTarea.obtenerPorVencer(numero)){
                acumulador = ("Por vencer");
            } else {
                acumulador = ("");
            }
        } else {
    ...
    public static String strFechaLimite(ListaTarea2 listaTarea,int numero){
        String acumulador;
        if (!listaTarea.esVacia()) {
    ...
    public static String mostrarTodo(ListaTarea2 listaTarea,int numero) {
        String acumulador;
        if (!listaTarea.esVacia()) {
            acumulador = (mostrar(listaTarea,numero)+ ". Prioridad " + listaTarea.obtenerPrioridad(numero) + ", Estado de " + strCompleta(listaTarea, numero) + ", Fecha límite " + strFechaLimite(listaTarea, numero)+ "."+ strPorVencer(listaTarea,numero));
        } else {
            acumulador = "Lista vacia.";
        }
        return acumulador;
    ...
    public static void mostrarTodas(ListaTarea2 listaTareas){
        int i=0;
    ...
    public static void mostrarTodasTodo(ListaTarea2 listaTareas){
        int i=0;
    ...
    public static int selecionarNumeroTarea(ListaTarea2 listaTareas){
    ...
    public static void cargarEjemplos(ListaTarea2 listaTareas){
        ...
        listaTareas.setFechaLimite(3,LocalDate.of(2024,1,12));
    ...
    public static void printMenu(){
        ...
        System.out.println("11. Establecer fecha recordatorio.");
